	...
	public String view(@RequestParam(value="id",required=false)String id,Model model) {
	  ...
	  @RequestMapping({"/movie/view/{id}"})
	    public String viewById(@PathVariable Optional<String> id, Model model) {
		  ...
		  if(!id.isPresent()) {
	            return "Err1";}
	        else if(archive==null) {return "Err1";}
	        else {
//	            MovieModel archive = movieService.getMovieDetail(id.get());
	            model.addAttribute("movie", archive);
	            return "view-movie";}}
	...
	public String viewAll(Model model) {
	  ...
	  @RequestMapping({"/movie/update/{id}/duration/{Duration}"})
	    public String upMovie(@PathVariable Optional<String> id,@PathVariable Optional<Integer> Duration, Model model) {
        MovieModel archive = movieService.getMovieDetail(id.get());
		  ...
		  if(!id.isPresent()) {
	        	...
	        	movieService.upMovie(id.get(), Duration.get());
//	            MovieModel archive = movieService.getMovieDetail(id.get());
	            model.addAttribute("movie", archive);
	            return "Update";}}
	  
	  @RequestMapping({"/movie/delete/{id}"})
	    public String delMovie(@PathVariable Optional<String> id,@PathVariable Optional<Integer> Duration, Model model) {
      MovieModel archive = movieService.getMovieDetail(id.get());
		  if(!id.isPresent()) {
	            return "Err1";}
	        else if(archive==null) {return "Err1";}
	        else {
	        	movieService.delMovie(id.get());
//	            MovieModel archive = movieService.getMovieDetail(id.get());
	            model.addAttribute("movie", archive);
	            return "Delete";}}
